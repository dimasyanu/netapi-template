...
public interface IMediaRepository : IHasOwnershipRepository
{
    ...
    Task<IReadOnlyList<MediaEntity>> GetFileListAsync(MediaFilter filter, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<string>> GetDirectoryListAsync(MediaFilter filter, CancellationToken cancellationToken = default);
